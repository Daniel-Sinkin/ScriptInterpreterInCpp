...
static void test_basic_two_lines()
{
    ...
    expect_token(tokens[2], TokenKind::OpEqual,     "=",     0, 6);
...
static void test_hspace_skipping()
{
    ...
    EXPECT_EQ(tokens[2].kind, TokenKind::OpEqual);
...
static void test_eof_only_once_and_at_end()
{
    ...
    for (const auto& t : tokens) {
        if (t.kind == TokenKind::Eof) ++eof_count;
    }
...
static void test_tokenize_range_absolute_line_col()
{
    ...
    // Slice out "PRINT x" only, but line/col should still be absolute:
