...
void BytecodeBuilder::patch_jump(IPtr where, IPtr target) {
    if (target == INVALIDIPtr) throw std::runtime_error("patch_jump: target INVALIDIPtr");

    auto& op = at(where);
    std::visit(
        overloaded{
            [&](BytecodeJmp& j) {
                if (j.target_ip != INVALIDIPtr) throw std::runtime_error("patch_jump: already patched");
                j.target_ip = target;
            },
            [&](BytecodeJmpFalse& j) {
                if (j.target_ip != INVALIDIPtr) throw std::runtime_error("patch_jump: already patched");
...
void BytecodeBuilder::begin_function_locals(const std::vector<std::string>& params) {
    ...
    for (usize i = 0; i < params.size(); ++i) {
        ...
        if (base.contains(p)) throw std::runtime_error("Duplicate parameter: " + p);
        base.emplace(p, static_cast<u32>(i));

        if (fn().seen_symbols.size() <= i) fn().seen_symbols.resize(i + 1);
        fn().seen_symbols[i] = p;
    }
...
void BytecodeBuilder::end_scope() {
    if (scopes_.empty()) throw std::runtime_error("end_scope: no scope");
    next_slot_ = scopes_.back().saved_next_slot;
    scopes_.pop_back();
}

u32 BytecodeBuilder::declare_local(std::string_view name) {
    if (scopes_.empty()) throw std::runtime_error("declare_local: no scope");
    const std::string key{name};

    auto& cur = scopes_.back().locals;
    if (cur.contains(key)) throw std::runtime_error("Variable already declared in this scope: " + key);
    ...
    if (fn().seen_symbols.size() <= u) fn().seen_symbols.resize(u + 1);
...
u32 BytecodeBuilder::resolve_func(std::string_view name) const {
    ...
    if (it == func_ids_.end()) throw std::runtime_error("Undefined function: " + key);
    return it->second;
}
...
void BytecodeBuilder::build_scope_statements(const std::vector<Statement>& sts) {
    for (const auto& st : sts) build_statement(st);
}
    ...
    std::visit(
        overloaded{
            ...
            [&](const UnaryExpression& e) {
                if (!e.rhs) throw std::runtime_error("UnaryExpression missing rhs");
                build_expression(*e.rhs);
                switch (e.op) {
                case UnaryOp::Neg: emit(BytecodeNEG{}); break;
                case UnaryOp::Not: emit(BytecodeNOT{}); break;
                }
            },
            ...
            [&](const BinaryExpression& e) {
                ...
                switch (e.op) {
                case BinaryOp::Add: emit(BytecodeAdd{}); break;
                case BinaryOp::Sub: emit(BytecodeSub{}); break;
                case BinaryOp::Mul: emit(BytecodeMult{}); break;
                case BinaryOp::Div: emit(BytecodeDiv{}); break;
                case BinaryOp::Mod: emit(BytecodeMod{}); break;

                case BinaryOp::Eq: emit(BytecodeEQ{}); break;
                case BinaryOp::Neq: emit(BytecodeNEQ{}); break;
                case BinaryOp::Lt: emit(BytecodeLT{}); break;
                case BinaryOp::Le: emit(BytecodeLE{}); break;
                case BinaryOp::Gt: emit(BytecodeGT{}); break;
                case BinaryOp::Ge: emit(BytecodeGE{}); break;
            ...
            [&](const CallExpression& e) {
                if (!e.callee) throw std::runtime_error("CallExpression missing callee");
                const auto* id = std::get_if<IdentifierExpression>(&e.callee->node);
                if (!id) throw std::runtime_error("CallExpression callee must be IdentifierExpression");

                for (const auto& arg : e.args) {
                    if (!arg) throw std::runtime_error("CallExpression has null arg");
                ...
                if (argc == 0) emit(BytecodeCall{fid});
                else emit(BytecodeCallArgs{fid, argc});
            },
        },
        expr.node);
}
    ...
    std::visit(
        overloaded{
            ...
            [&](const IntAssignmentStatement& s) {
                if (!s.expr) throw std::runtime_error("IntAssignmentStatement missing expr");
                const u32 slot = resolve_local(s.identifier);
                build_expression(*s.expr);
                emit(BytecodeStoreLocal{slot});
            },
            [&](const PrintStatement& s) {
                if (!s.expr) throw std::runtime_error("PrintStatement missing expr");
                build_expression(*s.expr);
                emit(BytecodePrint{});
                emit(BytecodePop{});
            },
            ...
            [&](const IfStatement& s) {
                if (!s.if_expr) throw std::runtime_error("IfStatement missing if_expr");
            ...
            [&](const WhileStatement& s) {
                if (!s.while_expr) throw std::runtime_error("WhileStatement missing while_expr");
