...
struct Parser
{
    ...
    [[nodiscard]] static std::optional<BpPair> infix_bp(const Token& tok) noexcept
    {
        if (auto bop = std::get_if<BinaryOperator>(&tok))
        {
            ...
            case BinaryOperator::Slash:            return BpPair{70, 71};

            case BinaryOperator::Plus:
            case BinaryOperator::Minus:            return BpPair{60, 61};

            case BinaryOperator::GreaterThan:
            case BinaryOperator::LessThan:
            case BinaryOperator::GreaterEqualThan:
            case BinaryOperator::LessEqualThan:    return BpPair{50, 51};

            case BinaryOperator::DoubleEqual:      return BpPair{40, 41};
            }
        }
        return std::nullopt;
    }
